...
# Print device of your PEFT model
    ...
    #for dialect, dataset in all_datasets.items():
        #raw_datasets[dialect] = raw_datasets.map(move_to_device, batched=True)
    ...
    for i, dialect in enumerate(data_args.dialects):
        ...
        with training_args.main_process_first(desc="dataset map pre-processing"):
            ...
            if not data_args.load_dataset:
                ...
                print(data_args.save_dataset_path[i])
...
##changed CPU to GPU here... Change back
        model = PeftModel.from_pretrained(model, "singe_lora_model", map_location=torch.device('gpu'))
    ...
    inputs = tokenizer(raw_datasets, return_tensors="pt").input_ids.to(device)
    ...
    if training_args.do_predict:
        ...
        print(predict_datasets.device)
        predict_datasets.move(device)
        if data_args.task_name == "mnli":
            tasks.append("mnli-mm")
